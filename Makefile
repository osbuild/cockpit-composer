...
# clean old end to end test container and build new end to end test image
prepare:
	sudo docker ps --quiet --all --filter 'ancestor=welder/web-e2e-tests' | sudo xargs --no-run-if-empty docker rm -f
	sudo docker build -f ./test/end-to-end/Dockerfile -t welder/web-e2e-tests:latest ./test/end-to-end/

# run the end to end test
check: $(VM_IMAGE) prepare
